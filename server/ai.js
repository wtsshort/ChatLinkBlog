...
export async function generateArticle(topic, language = 'ar') {
  try {
...
export async function generateSEOData(content, language = 'ar') {
  try {
...
export async function generateImagePrompt(articleTitle, language = 'ar') {
  try {
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
